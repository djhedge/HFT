...
namespace FIXApi
{
    ...
    {
        StockTrending, StockFishing, CBFishing, CBSpread, alpha_arbitrage_HS300, alpha_arbitrage_ZZ500, alpha_arbitrage_SZ50, alpha_mamount_ZZ500,alpha_mamount_TotalA,Other
    }
    ...
    public class FIXTrade : QuickFix.MessageCracker, QuickFix.Application
    {
                    ...
                    if (OrderBook.ContainsKey(ClOrdID))
                    {
                        switch (report.getInt(39))
                        {
                                ...
                                lock (this.dataDict)
                                {
                                    ...
                                    else if (OrderBook[ClOrdID].strategies == TradingStrategies.StockTrending)
                                    {
                                        ...
                                        if ((OrderBook[ClOrdID].action == OrderAction.Lock)
                                            || (OrderBook[ClOrdID].action == OrderAction.ShortSell)) //三种空单：锁券、撤锁挂单、改价重挂
                                            dataDict[ticker].StatusTrending = StrategyStatus.ShortListedOnly;
                                        else if (OrderBook[ClOrdID].action == OrderAction.Buy) //一种多单
                            ...
                            case 1: //"部成" //注意部成可能会多次返回， 产生不同的数量和均价，最好用数组来存储每次成交的数量和均价
                                ...
                                {
                                    //只有三种单子：Lock，LockAndBu,ShortSell,其中Lock不可能出现部成，Buy也基本不可能出现部成
                                    string ticker = OrderBook[ClOrdID].ticker;
                                    //部成也当成交处理，开始挂空单，后面剩余多单由于限价较高，都会成交掉，影响不大。
                                    ...
                                    else if ((OrderBook[ClOrdID].action == OrderAction.ShortSell))
                                    {
                            ...
                            case 2: // "成交"
                                ...
                                {
                                    //只有三种单子：Lock，LockAndBuy，ShortSell，其中Lock不可能出现成交
                                    string ticker = OrderBook[ClOrdID].ticker;
                                    if (OrderBook[ClOrdID].action == OrderAction.Buy)
                                    {
                                        lock(this.dataDict)
        ...
        void Application.fromApp(Message message, SessionID sessionID)
        {
            ...
            if (message.getHeader().getString(35) == "UAP")
            {
                ...
                if (message.getField(724) == "0")       // 股份
                {
                    ...
                    //需匹配
                    dt_hold.Rows[dt_hold.Rows.Count - 1][15] = "dj1";
       ...
       public void WriteToServer(DataTable dt)
        {
            ...
            conn.Open();
           //删除该账户持仓
            string x = "delete from StockHold where 账户='" + Convert.ToString(dt.Rows[0][15]) + "' and 股东账号= '" + Convert.ToString(dt.Rows[0][0])+"'";
            qcmd.CommandText = "delete from StockHold where 账户='" + Convert.ToString(dt.Rows[0][15]) + "' and 股东账号= '" + Convert.ToString(dt.Rows[0][0]) + "'";
     ...
     public  void OrderBookUpdate(string tablename)
        {
           ...
           //OBE.orderId = "2222";
           //OBE.orderDate = Convert.ToInt32(DateTime.Now.ToString("yyyyMMdd")) +1;
           //OrderBook.Add("2222", OBE);   
     ...
     public void PositionUpdate()
     {
         ...
         dtPosition.Columns.Add("Strategy");
         dtPosition.Columns.Add("SubStrategy1");
         dtPosition.Columns.Add("SubStrategy2");
         dtPosition.Columns.Add("Ticker");
         dtPosition.PrimaryKey = new DataColumn[] { dtPosition.Columns["Account"], dtPosition.Columns["Strategy"], dtPosition.Columns["SubStrategy1"], dtPosition.Columns["SubStrategy2"], dtPosition.Columns["Ticker"] };
         ...
         while (sdr.Read())
         {
             ...
             if (!Hasrecord)
             {
                 DataRow newdr = dtPosition.NewRow();
                 newdr["Account"] = account;
                 newdr["Ticker"] = ticker;
                 if (type=="Buy")
                     newdr["PrePosition"] = num;
                 else
                     newdr["PrePosition"] = -num;
                 newdr["TodayBuy"] = 0;
                 newdr["TodaySell"] = 0;
         ...
         {
             DataRow dr = dtPosition.Rows[i];        
             dr["PrePosition"] = Convert.ToInt32(dr["PrePosition"]) + Convert.ToInt32(dr["TodayBuy"]) - Convert.ToInt32(dr["TodaySell"]);          
             dr["TodayBuy"] = 0;
             dr["TodaySell"] = 0;
             if (Convert.ToInt32(dr["PrePosition"]) == 0)
             {
                 //状态为Added,删除一行后，后面的会往前移
        ...
        public void OrderSender_StockTrending(object oEntry)
        {
           ...
           // if ((mode == TradeMode.Backtest) || (mode == TradeMode.Debug))
            if ((mode == TradeMode.Backtest))
            {
                ...
                if (entry.action == OrderAction.Buy)
                {
                    dt_PnL.Rows.Add(new object[] {tradeDate, entry.ticker, entry.orderTime.ToString(), 
         entry.priceListed.ToString(), entry.quantityListed.ToString(),"0","0","0","0","0"});
                }
                else if (entry.action == OrderAction.ShortSell)
                {
            ...
            else //以下为生产环境
            {
                ...
                else if (entry.action == OrderAction.Buy) //先挂单锁券
                {
                    string bufferUsed = "";
                    ...
                    */
                    //下达买单指令
                    if ((entry.action == OrderAction.Buy)) //正常买
                    {
                        //成交价格和成交量由成交回报来填写
                        lock (this.dt_PnL)
                        {
                            dt_PnL.Rows.Add(new object[] { tradeDate, entry.ticker, entry.orderTime.ToString(), "0", "0", "0", "0", "0", "0", "0" });
                        }
